...
def source_flow_constraint(m, source_node, variable, G):  # RENAME THIS CONSTRAINT
    ...
    link_variable = get_model_chars.get_link_variable(m, variable)
...
def get_up_treatment_equation(m, unit_process, up_edge, variable, G):
    ...
    up = module_import.get_module(G.edges[up_edge]["treatment_name"])
    link_variable = get_model_chars.get_link_variable(m, variable)
...
def calculate_recovered_streams(m, unit_process, variable, G):
    ...
    sum_of_treated_flow = get_sum_of_up_outflow(
        m, unit_process, "use_stream", variable, G
    )
...
def max_recycle_streams(m, unit_process, variable, G):
    ...
    sum_of_recycle_flow = get_sum_of_up_outflow(
        m, unit_process, "recycle_stream", variable, G
    )
    link_variable = get_model_chars.get_link_variable(m, variable)
    ...
    if sum_of_recycle_flow is 0:
        ...
        for edge in G.in_edges("%s_source_and_recycle" % unit_process):
            if G.edges[edge]["type"] == "feedwater_stream":
                sum_of_feedwater = (
                    sum_of_feedwater + link_variable[G.edges[edge]["name"]]
                )
...
def calculate_recovered_water(m, end_node, G):  # AT NODE
    ...
    for up_outflow_link in G.in_edges(end_node):  # TO DO MAKE FUNCTION
        recovered_flow = (
            recovered_flow + m.FlowInLinkSegments[G.edges[up_outflow_link]["name"]]
        )
...
def calculate_total_train_cost(m, end_node, G):  # AT NODE
    ...
    for unit_process in get_graph_chars.get_unit_process_name_list(G):
...
def calculate_levelized_cost(m, end_node):
    return (
        m.LevelizedCost[end_node]
        == m.RecoveredCost[end_node] / m.RecoveredFlow[end_node]
    )
...
def calculate_up_total_costs(m, unit_process, G, cost_method):
    ...
    if mem_manu is not None:
        return m.TotalCostInLinkSegments[G.edges[up_edge]["name"]] == up.total_up_cost(
            m=m,
            flow_in=flow_in,
            G=G,
        ...
        return m.TotalCostInLinkSegments[G.edges[up_edge]["name"]] == up.total_up_cost(
            flow_in=flow_in, G=G, cost_method=cost_method
        )
