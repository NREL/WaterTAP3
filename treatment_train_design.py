...
unit_process_library_list = wt.unit_process_library_list


### DESCRIBE todo
# MOVE THE BELOW SO THAT IT CAN BE CALLED FROM NOTEBOOK
def create_train(unit_process_name_list,
                #unit_process = False,
    ...
    G = add_multiple_unit_processes(G, 
...
def add_flows(G, flow, source_nodes = None):
    ...
    if source_nodes is None: source_nodes = get_graph_chars.get_source_nodes(G)
...
def add_recycle_stream(G, 
    ...
    if recyle_fraction_of_waste is not None:
        ...
        recycle_to_start_edge = (recycle_node_name, ('%s_start' % unit_process_name),
                               {'name': ('%s_source_and_recycle_stream' % unit_process_name),
                               'type': 'source_and_recycle_stream'}) 

        treatment_edge = (('%s_start' % unit_process_name), ('%s_end' % unit_process_name),
                                       {'name': unit_process_name, 
                                       'type': 'treatment_process',
                                       'treatment_name' : unit_process})

        end_to_recycle_edge = (('%s_end' % unit_process_name), recycle_node_name,
        ...
        G = update_unit_process_recycle(G, unit_process, unit_process_name, recyle_fraction_of_waste)
...
def add_multiple_unit_processes(G, 
    ...
    for unit_process, unit_process_name in zip(unit_process_list, unit_process_name_list):
        ...
        # ADD THE UNIT PROCESSES AND LINKS
        source_edge = (source_node_name, ('%s_start' % unit_process_name),
                                   {'name': ('%s_feedwater' % unit_process_name), 
        ...
        waste_edge = (('%s_end' % unit_process_name), ('%s_waste' % unit_process_name),
                                   {'name': ('%s_waste_stream' % unit_process_name), 
                                   'type': 'waste_stream'})

        product_edge = (('%s_end' % unit_process_name), end_node_name,
                                   {'name': ('%s_use_stream' % unit_process_name), 
                                   'type': 'use_stream'})

        G.add_edges_from([source_edge, treatment_edge, waste_edge, product_edge])
    
    
    #wt.display.treatment_train_graph(G)
    ...
    if source_water_reference is not None:
        G = get_source_data(G, source_water_reference, source_water_type, source_water_case_study, source_node_name)
    
    if enduse_water_reference is not None:
        G = get_enduse_data(G, enduse_water_reference, enduse_water_type, enduse_water_case_study, end_node_name)
        
    return G



def get_source_data(G, 
                    source_water_reference, 
                    source_water_type, 
                    source_water_case_study,
                    source_node_name = None): # need to do this to allow multiple at a time.
    

    #### READ IN SOURCE DATA
    df_source = pd.read_csv('case_study_water_sources_and_uses.csv')
    ...
    for variable in df_source.Variable:
        G.nodes[source_node_name][variable] = df_source[df_source.Variable == variable].mean().max()
    
    return G

def get_enduse_data(G, 
    ...
    df_enduse = pd.read_csv('case_study_water_sources_and_uses.csv')    
    ...
    for variable in df_enduse.Variable:
        G.nodes[enduse_node_name][variable] = df_enduse[df_enduse.Variable == variable].mean().max()
...
def update_feedwater_data(df,
    ...
    for n in range(0, number_of_additional_sources): # need to test adding more than one at a time
        ...
        if from_case_study == True:
            feedwater_data_new.Value = feedwater_data_new.Variable.map(df.Value) # CHANGE THIS FEEEDWATER WITH AN ATUAL CASE STUDY NAME OR WAY TO LOOKUP
...
def add_multiple_water_sources(G,
                               source_water_reference = None,
                               source_water_type = None,
                               source_water_case_study = None,
    ...
    for source_name, to_node in source_to_node_names:
        ...
        G.add_edges_from([(source_name, to_node,
                           {'name': ('%s_%s_feedwater' % (source_name, to_node)), 
                            'type': 'feedwater_stream'})])

    G = name_nodes(G)
    
    G = get_source_data(G, source_water_reference, source_water_type, source_water_case_study, source_node_name = source_name)
...
def add_multiple_water_enduses(G,
                               enduse_water_reference = None,
                               enduse_water_type = None,
                               enduse_water_case_study = None,
    ...
    for from_node, end_name in node_to_end_names:
        ...
        G.add_edges_from([(from_node, end_name, 
                       {'name': ('%s_treated_%s_stream' % (from_node, end_name)),
                       'type': 'use_stream'})])
        
    G = name_nodes(G)
    
    G = get_enduse_data(G, enduse_water_reference, enduse_water_type, enduse_water_case_study, enduse_node_name = end_name)
...
################################################################
################################################################
#### GET INFORMATION FOR UNIT PROCESSES AND APPLY TO TRAIN ####

def update_unit_process_recycle(G,
                  unit_process,              
                  unit_process_name,
    ...
    for edge in G.edges():
        if G.edges[edge]['name'] == unit_process_name:
            up = G.edges[edge]['treatment_name']
            up_name = G.edges[edge]['name']
            UP = wt.module_import.get_module(up)
            
            print('updating unit process attributes:', unit_process_name)
                
            G = add_unit_attributes(UP, G, up, up_name, recyle_fraction_of_waste)  
        
    return G
    

def set_up_unit_process(G):
    # eventualy this should have a UNITPROCESS.run_all() -> run_all should run each step in the process.
    import get_graph_chars

    unit_processes_in_train = get_graph_chars.get_unit_process_name_list(G)
    
    for edge in G.edges():
        if G.edges[edge]['type'] == 'treatment_process':
            recyle_fraction_of_waste = 0 # automatic set and readjusts below
            up = G.edges[edge]['treatment_name']
            up_name = G.edges[edge]['name']
            if 'recycle_factor' in list(G.edges[edge]):
                if G.edges[edge]['recycle_factor'] > 0: 
                    recyle_fraction_of_waste = G.edges[edge]['recycle_factor'] / (1 - G.edges[edge]['recovery_factor'])
            
    #for up in unit_processes_in_train:
            print('adding unit process attributes:', up)
...
def add_unit_attributes(UP, G, up, up_name, recyle_fraction_of_waste = None):
    # THIS NEEDS TO BE AN ADD ALL ATTRIBUTES
    G = UP.add_recovery_attribute(G, up, up_name)
    G = UP.add_BOD_inlet_constraint(G, up, up_name)
    G = UP.add_TOC_inlet_constraint(G, up, up_name)
    G = UP.add_recycle_and_waste_attribute(G, up, up_name, recyle_fraction_of_waste)

    return G


def add_unit_process(G, 
                    from_node = None,
                    to_node = None,
    ...
    if unit_process_list == None: unit_process_list == unit_process_name_list
    # ADD THE UNIT PROCESSES AND LINKS
    ...
    for unit_process, unit_process_name in zip(unit_process_list, unit_process_name_list):
        ...
        source_edge = (from_node, ('%s_start' % unit_process_name),
                                   {'name': ('%s_feedwater' % unit_process_name), 
                                   'type': 'feedwater_stream'})
        ...
        product_edge = (('%s_end' % unit_process_name), end_use_node_name,
                                   {'name': ('%s_use_stream' % unit_process_name), 
                                   'type': 'use_stream'})
...
def add_stream(G, from_node = None,
                to_node = None,
                stream_name = None,
                stream_type = None,):
    
    if stream_name is None: print('input error likely: stream_name not set')
