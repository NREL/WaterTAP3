...
def create_arcs(m, arc_dict):
	for key in arc_dict.keys():
		...
		setattr(m.fs, ("arc%s" % key), Arc(source=getattr(getattr(m.fs, source), source_port), destination=getattr(getattr(m.fs, outlet), outlet_port)))
		...
		if pfd_dict[key]["Type"] == "intake":
			...
			for node in range(0, len(pfd_dict[key]["Parameter"]["source_type"])):
				...
				if source_exists == False:
					m = wt.design.add_water_source(m=m, source_name=source_name, reference=reference, water_type=water_type, case_study=case_study, flow=flow)
...
def create_mixers(m, mixer_list, arc_dict, arc_i):
	...
	for j in mixer_list:
		...
		setattr(m.fs, mixer_name, Mixer1(default={"property_package": m.fs.water, "inlet_list": inlet_list}))
...
def add_wate_streams(m, arc_i, pfd_dict, mixer_i):
	...
	if "surface_discharge" in pfd_dict.keys():
		...
		if len(waste_inlet_list) > 1:
			...
			setattr(m.fs, waste_mixer, Mixer1(default={"property_package": m.fs.water, "inlet_list": waste_inlet_list}))
				...
				if hasattr(b_unit, 'waste'):
					...
					if len(getattr(b_unit, "waste").arcs()) == 0:
						...
						if str(b_unit)[3:] in list(pfd_dict.keys()):
							if pfd_dict[str(b_unit)[3:]]["Type"] == "treatment":
								i = i + 1
								setattr(m.fs, ("arc%s" % arc_i), Arc(source=getattr(b_unit, "waste"), destination=getattr(getattr(m.fs, waste_mixer), "inlet%s" % i)))
								arc_i = arc_i + 1

			# add connection for waste mixer to surface dicharge -->
			if "surface_discharge" in list(pfd_dict.keys()):
				setattr(m.fs, ("arc%s" % arc_i), Arc(source=getattr(m.fs, waste_mixer).outlet, destination=getattr(m.fs, "surface_discharge").inlet))
				arc_i = arc_i + 1
