...
def create(m, unit_process_type, unit_process_name):
    ...
    scenario_list = df[df.unit_process == unit_process_type].scenario.to_list()
    default_df = df[((df.unit_process == unit_process_type) & (df.case_study == 'default'))].recovery

    if case_study_name in case_study_list and scenario in scenario_list:
        case_study_df = df[((df.unit_process == unit_process_type) & (df.case_study == case_study_name) & (df.scenario == scenario))].copy()
