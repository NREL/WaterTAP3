...
class UnitProcess(WT3UnitProcess):
    ...
    def get_costing(self, unit_params=None, year=None):
        ...
        # ((((flow_in_gal_day / (self.num_ix_units_op[t])) / 1440) / self.loading_rate[t]) * 1.22)
        # )
        #
        ...
        #        self.num_units_constr = Constraint(expr=self.num_ix_units_op[t] == self.num_ix_units_tot[t] - 1) # number operating units is one less than total units -- assume one is offline at
        #        all times for
        # regeneration
        ...
        #         num_redund_small_1 = 1  # Number of redundant vessels for small systems with only one vessel -- If a design includes only one operating vessel, there should be at least one
        #         additional vessel
        #         num_redund_small = 0  # Number of redundant vessels for small systems with multiple vessels -- Small systems can operate at reduced capacity while one vessel is down for backwash,
        #         if there are multiple vessels
        #         redund_freq = 4  # Add a redundant unit per x parallel lines
        #         bp_pct = 0.15  # * pyunits.dimensionless # bypass percent of flow -- Typically, bypass flow ranges between 0% and 50% of inflow. Enter zero to remove bypass piping and components.
        #         0% is default consistent with the type of cost curves that EPA will need for its national cost simulation model.
        ...
        #         max_height = 14  # * pyunits.ft # Maximum vessel height (circular vessels) -- Systems have used vessels up to 14' diam and height in practice (AWWA Inorganic Contaminants Workshop
        #         2006)
        #         # max_diam_override =
        #         max_length = 53  # * pyunits.ft #Maximum vessel length (longitudinal vessels) -- Maximum vessel diameter will be chosen from a table of conventional sizes for commercial vessels,
        #         unless entered here.  Systems have used vessels up to 14' diam and height in practice (AWWA Inorganic Contaminants Workshop 2006)
        ...
        #         vessel_thickness = 0  # * pyunits.ft # thickness of vessel walls ? -- Set to zero because vendor designs and quotes are usually expressed in inside diameter.  Increase if user
        #         desires to input outside diameter.
        ...
        #         min_bed_depth = 2.5  # * pyunits.ft # minimum bed depth for autosize -- Guidance: Typically bypass flow ranges between 10 % and 50 % of inflow.  Enter zero to remove bypass piping
        #         and components. 0%
        ...
        #         regen_rate = 0.8  # * (pyunits.gallon / pyunits.minute) / pyunits.ft**3 # regen loading rate -- Minimum brining rate suggested by resin manufacturers to avoid channelling is 0.25
        #         gpm/ft3 of resin, suggest 0.5 to 0.8 (Boodoo Peer Review Comments)
        #         regen_reuse = 0  # * pyunits.dimensionless # number of times regen to be reuse; can be reused up to 25 times -- With brine storage/mixing tanks, regenerant may be reused up to 25
        #         times.  Cannot reuse regenerant with salt saturator as the delivery method -- additional tanks would be required.
        ...
        #         brine_conc = 0.4  # * (pyunits.lb / pyunits.gal) # regen brine concentration -- Designers typically choose a brine concentration that is less than 10%, usually as low as 6% to
        #         allow for adequate contact time with the resin during the brining step (Boodoo Peer Review comments)
        ...
        # _______________________________________________________  # _______________________________________________________  ############## ACTUALLY CALCULATING COST ##############  #
        ...
        #                         min_diam_a = 2 * ((
        #                                                   comp_vol_required / num_tanks / 3.14159 / comp_max_vert_bed_depth / num_lines) ** 0.5 + vessel_thickness)  # min diam to avoid bed depth
        #                                                   becoming too deep, given required volume
        ...
        #                     comp_min_vert_height, comp_max_vert_height, comp_min_vert_diam, comp_max_vert_diam, comp_min_vert_bed_depth, comp_max_vert_bed_depth = autosize_constraints(
        #                     system_type, geom=geom)
        ...
        #                     comp_min_horiz_diam, comp_max_horiz_diam, comp_min_horiz_length, comp_max_horiz_length, comp_min_horiz_bed_depth, comp_max_horiz_bed_depth = autosize_constraints(
        #                     system_type,
        #                                                                                                                                                                                       geom=geom)
        #                     # inputs
        #                     comp_max_vol = 27100  # max pressure vessel volume LOOKED UP
        #                     comp_max_vol_length = comp_max_vol / 7.48 / (3.14159 * comp_max_horiz_diam * comp_max_horiz_diam / 4)  # rounded to reflect the length we will actually use on the
        #                     input sheet
        ...
        #             if system_type == 'gravity':
        #                 comp_min_bed_depth_a, comp_max_bed_depth_a, comp_min_sa_a, comp_max_sa_a, comp_min_length_a, comp_max_length_a, comp_min_width_a, comp_max_width_a = autosize_constraints(
        #                 system_type,
        #                                                                                                                                                                                           geom=geom)
