...
def get_all_none_source_end_nodes(G):
    node_list = []
    for node in G.nodes():
        if ((len(G.in_edges(node)) >= 1) & (len(G.out_edges(node)) >= 1)): node_list.append(node)
...
def get_source_nodes(G): # need to diffrentiate source nodes in some way if there are multiple source nodes.
    ...
    for node in G.nodes():
        if G.nodes[node]['type'] == 'source':
            if node not in source_node_list: source_node_list.append(node)
...
def get_recovered_water_nodes(G):
    ...
    for node in G.nodes():
        if G.nodes[node]['type'] == 'use':
            if node not in enduse_node_list: enduse_node_list.append(node)
