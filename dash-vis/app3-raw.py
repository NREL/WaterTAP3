...
### Preload
df = pd.read_csv('./Carlsbad_Baseline_results.csv', index_col=0)
df.rename(columns={"Unit Process Name": "Unit_Process", "Case Study": "Case_Study"}, inplace=True)

all_case = df['Case_Study'].unique()
metric_dict = {'COST':'Cost', 'ELEC':'Electricity', 'WATER':'Water Flow', 'CHEM':'Constituent Flow'}
        ...
        html.Div([
            ...
            dcc.Checklist(
                ...
                options=[ 
                    ...
                    {'label': 'Constituent Flow', 'value': 'CHEM'}, 
                ],
...
def set_unit_process_options(selected_metric_type, selected_scenario, selected_case):
    ...
    if selected_metric_type is None:
        ...
        if 'CHEM' in selected_metric_type:
            ddf2_new = pd.concat([ddf2_new, ddf2[ddf2['Metric']=='Constituent Flow']])    
...
def update_graph(stack_on, percentage_on, selected_variable, selected_unit_process, 
    ...
    ### Select data
        ...
        if len(selected_metric_type) == 1:
            ...
            fig.update_yaxes(title='%s' % (metric_name), type='linear')
            fig.update_layout(title_text="%s Result I" % (metric_name))
            ...
            fig.update_yaxes(title_text='%s'%(metric_name1), secondary_y=False)
            fig.update_yaxes(title_text='%s'%(metric_name2), secondary_y=True)
            fig.update_layout(barmode='stack', title_text="Combined Results I")
...
def update_graph2(stack_on, percentage_on, selected_variable, selected_unit_process, 
    ...
    if (selected_unit_process is None) or (selected_variable is None):
        ...
        if len(selected_metric_type) == 1:
            ...
            fig.update_xaxes(title=x_name)
            fig.update_yaxes(title='%s' % (metric_name), type='linear')
            fig.update_layout(title_text="%s Result II" % (metric_name))
            ...
            fig.update_yaxes(title_text='%s'%(metric_name1), secondary_y=False)
            fig.update_yaxes(title_text='%s'%(metric_name2), secondary_y=True)
            fig.update_layout(barmode='stack', title_text="Combined Results II")
...
def update_graph3(stack_on, percentage_on, selected_variable, selected_unit_process, 
    ...
    if (selected_unit_process is None) or (selected_variable is None):
        ...
        if len(selected_metric_type) == 1:
            ...
            fig.update_yaxes(title_text='%s'%(metric_name1), secondary_y=False)
            fig.update_yaxes(title_text='%s'%(metric_name2), secondary_y=True)
            fig.update_layout(barmode='stack', title_text="Combined Results Difference")
