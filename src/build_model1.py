...
def build_model(G, model, cost_method="wt", optimize_path=False):
    ...
    model.LinkSet = env.Set(
        initialize=wt.get_graph_chars.get_link_list(G)
    )  # all links in network
    model.UnitProcesses = env.Set(
        initialize=wt.get_graph_chars.get_unit_process_name_list(G)
    )  # all unit processes
    model.SourceNodes = env.Set(
        initialize=wt.get_graph_chars.get_source_nodes(G)
    )  # all source nodes
    model.AllNoneSourceEndNodes = env.Set(
        initialize=wt.get_graph_chars.get_all_none_source_end_nodes(G)
    )
    model.RecoveredWaterNodes = env.Set(
        initialize=wt.get_graph_chars.get_recovered_water_nodes(G)
    )
    ...
    model.CalculateRecoveredWater = env.Constraint(
        model.RecoveredWaterNodes2, rule=mc.calculate_recovered_water
    )

    model.CalculateRecoveredCost = env.Constraint(
        model.RecoveredWaterNodes2, rule=mc.calculate_total_train_cost
    )
    ...
    for variable in water_variables:
        for i in range(0, len(wt.get_graph_chars.get_all_none_source_end_nodes(G))):
            x1.append(
                [wt.get_graph_chars.get_all_none_source_end_nodes(G)[i], variable, G]
            )
        ...
        for i in range(0, len(wt.get_graph_chars.get_unit_process_name_list(T))):
            x3.append(
                [wt.get_graph_chars.get_unit_process_name_list(G)[i], variable, G]
            )
    ...
    model.NodeMassBalanceConstraint = env.Constraint(
        model.AllNoneSourceEndNodes2, rule=mc.node_mass_balance_constraint
    )
    model.CheckUnitInletFlows = env.Constraint(
        model.SourceNodes2, rule=mc.source_flow_constraint
    )
    model.CalculateRecoveredFlows = env.Constraint(
        model.UnitProcesses2, rule=mc.calculate_recovered_streams
    )

    # RECYCLE
    model.CalculateRecycleRecoveredFlows1 = env.Constraint(
        model.UnitProcesses2, rule=mc.max_recycle_streams
    )
    ...
    if len(wt.get_graph_chars.get_unit_process_name_list(G)) > 1:
        for i in range(0, len(wt.get_graph_chars.get_unit_process_name_list(G))):
            x1.append(
                [wt.get_graph_chars.get_unit_process_name_list(G)[i], G, cost_method]
            )
    else:
    ...
    model.CalculateUPCosts = env.Constraint(
        model.UnitProcesses3, rule=mc.calculate_up_total_costs
    )
    ...
    model.CalculateLevelizedCost = env.Constraint(
        model.RecoveredWaterNodes, rule=mc.calculate_levelized_cost
    )
...
def build_model1(G, cost_method="wt", optimize_path=False):
    ...
    model.RecoveredWaterNodes2 = env.Set(initialize=x1)  # all recovered water
    model.CalculateRecoveredWater = env.Constraint(
        model.RecoveredWaterNodes2, rule=calculate_recovered_water
    )

    model.CalculateRecoveredCost = env.Constraint(
        model.RecoveredWaterNodes2, rule=calculate_total_train_cost
    )
    ...
    # CONSTRAINTS FOR FLOW
    model.NodeMassBalanceConstraint = env.Constraint(
        model.AllNoneSourceEndNodes2, rule=node_mass_balance_constraint
    )
    model.CheckUnitInletFlows = env.Constraint(
        model.SourceNodes2, rule=source_flow_constraint
    )
    model.CalculateRecoveredFlows = env.Constraint(
        model.UnitProcesses2, rule=calculate_recovered_streams
    )

    # RECYCLE
    model.CalculateRecycleRecoveredFlows1 = env.Constraint(
        model.UnitProcesses2, rule=max_recycle_streams
    )
    ...
    model.UnitProcesses3 = env.Set(initialize=x1)
    model.CalculateUPCosts = env.Constraint(
        model.UnitProcesses3, rule=calculate_up_total_costs
    )
    ...
    model.CalculateLevelizedCost = env.Constraint(
        model.RecoveredWaterNodes, rule=calculate_levelized_cost
    )
