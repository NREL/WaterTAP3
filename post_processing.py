        ...
        for edge in G.edges():
            if G.edges[edge]['type'] == 'treatment_process':
                if G.edges[edge]['mem_manu'] is not None:
                    if env.value(model.FlowInLinkSegments[G.edges[edge]['name']]) > 0.000001:
                        df['mem_manu'] = np.array(None)
                        df['mem_model_type'] = np.array(None)
                        df.at[G.edges[edge]['name'], 'mem_manu'] = G.edges[edge]['mem_manu']
                        df.at[G.edges[edge]['name'], 'mem_model_type'] = G.edges[edge]['mem_model_type']
                        
    return df    
    
    
def get_bar_chart(data = None, x_variable = None, y_variable = None, 
                  compare_trains = False, models = None, model_names = None, compare_scenario = None):
    ...
    if data is not None:
        ...
        if x_variable == None: x_variable = 'link'
        ...
        if compare_scenario is None:
            ...
            figure = figure.set_xticklabels(figure.get_xticklabels(), rotation=45, horizontalalignment='right', 
                                           fontweight='light');
        
        else:
            
            figure = sns.barplot(x=x_variable, y=y_variable, data=data, hue = compare_scenario, ci = None);
            figure = figure.set_xticklabels(figure.get_xticklabels(), rotation=45, horizontalalignment='right', 
                                           fontweight='light');
    ...
    if models is not None:
        ...
        plt.figure(figsize=(16, 6))
        figure = sns.barplot(x=x_variable, y=y_variable, data=data, hue = 'Model', ci = None);
        figure = figure.set_xticklabels(figure.get_xticklabels(), rotation=45, horizontalalignment='right', 
                                       fontweight='light');         
