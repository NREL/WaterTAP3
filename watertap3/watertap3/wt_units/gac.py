...
class UnitProcess(WT3UnitProcess):
    ...
    def get_costing(self, unit_params=None, year=None):
        ...
        storage_conc_vol = ((
                                    2 * storage_basin_depth * storage_basin_length + 2 * storage_basin_depth * storage_basin_width + storage_basin_width * storage_basin_length) * 1.2 * conc_thick *
                            storage_basins) / 27  # * pyunits.yard ** 3
        ...
        dredging_freq = 1  # * pyunits.year * -1 # Evaporation pond solids removal frequency -- Range from 1 to 3 yrs, depending on the final solid concentration required and local climate (
        # Dewatering Municipal Wastewater Sludge,EPA design Manual, Dewatering Municipal Wastewater Sludges.EPA/625/1-87/014, September 1987)
        ...
        # hexcavation_vol = ((hbasins * hbasin_width + (hbasins + 1) * conc_thick + overexcavate) * (hbasin_length + 2 * conc_thick + overexcavate) * (hbasin_depth + conc_thick + overexcavate) + 
        # 0.5 * (
        #             3 * (hbasin_depth + conc_thick + overexcavate) * (hbasin_depth + conc_thick + overexcavate) * (hbasin_length + 2 * conc_thick + overexcavate))) / 27 # * pyunits.yard ** 3 # 
        #             Excavation volume
        # hbackfill_vol = hexcavation_vol - (hbasins * hbasin_width + (hbasins + 1) * conc_thick) * (hbasin_length + 2 * conc_thick) * (hbasin_depth + conc_thick) / 27 # * pyunits.yard ** 3 # 
        # Backfill volume
