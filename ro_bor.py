...
rejection_intrinsic = .996 # T16
rejection_obs = .995592074544 # T17, from BOR Rejection sheet, last iteration in iteration table, test solution
rejection_apparent = .995592074544 # T18, site concentration
...
transfer_pumps_motor_eff = .94
transfer_pumps_pump_eff = .75
...
product_pumps_motor_eff = .94
product_pumps_pump_eff = .75
...
concentrate_cost_per_L = .013
sitework_cost_per_m3 = 14.53 

# Indirect Capital Costs (sheet g)
interest_during_construction_percent = .05
contingencies_percent = .05976
fees_percent = .1195
working_capital_percent = .04

# Read in the csv that lists the constituent and their mg/L, molecular weight, and moles/L
df = pd.read_csv('bor_constituents.csv', index_col = False)

mem_char_list = ['mem_area_per_module',
...
def total_up_cost(m = None, G = None, flow_in = 0, cost_method = 'wt', mem_manu = None, mem_model_type = None, up_edge=None):
    ...
    feed_flow_rate = (flow_in * 0.0115741) # module calcs below need to be in liters per second. this is m3/d -> l/s
    ...
    mem_conc_salt_in_product = mem_conc_salt_in_feed * (1 - mem_rejection_chloride) # mol/m^3
    mem_conc_salt_in_reject = (mem_conc_salt_in_feed - mem_conc_salt_in_product * mem_recovery_rate) / (1 - mem_recovery_rate) # mol/m^3
    mem_conc_bulk = (mem_conc_salt_in_feed + mem_conc_salt_in_reject) / 2

    mem_osmotic_pressure = NaCl_diss_const * 2 * 8.314 * (273.15 + mem_temp) * (mem_conc_bulk - mem_conc_salt_in_product) * (1-rejection_obs)/((1-rejection_intrinsic) * 1000) #kPa
    ...
    conc_salt_in_reject = (conc_salt_in_feed - conc_salt_in_product * recovery_factor) / (1 - recovery_factor) # mole/m^3
    conc_bulk = (conc_salt_in_feed + conc_salt_in_reject) / 2 # mole/m^3
    
    # Operating Pressure (sheet f)
    T_coeff = 1.023**(feed_T - 25)
    osmotic_pressure = mem_rejection_chloride * 8.314 * (273.15 + feed_T) * (conc_bulk - conc_salt_in_product) * (1-rejection_apparent)/((1-rejection_intrinsic) * 1000) #kPa
    applied_pressure = osmotic_pressure + net_driving_pressure #kPa
    
    # Transfer pumps (to HPP) (sheet f)
    HPP_capacity = feed_flow_rate / HPP_number # capacity per pump, also = flow rate per skid, L/s
    HPP_capacity_m3 = HPP_capacity / 1000
    pipe_x_area = HPP_capacity_m3 / 2.5 # cross sectional area m^2, (Ax=Q/v), be sure x-section is adequate for the flow (v ~ 8.2 ft/s)
    HPP_size = (HPP_height_diff * 9.81 + (((HPP_capacity_m3 / pipe_x_area)**2) / 2) + (applied_pressure-206)) * (1-ERG_eff) * HPP_capacity / (746 * HPP_motor_eff * HPP_pump_eff * HPP_coupling_eff**-1) # hp
    
    # Transfer Water Pump (sheet f)
    transfer_pumps_capacity = feed_flow_rate / transfer_pumps_number # L/s per pump
    transfer_pumps_area = transfer_pumps_capacity / (1000 * 2.5) # m^2 pipe cross sectional area
    
    if transfer_pumps == 'Y':
        transfer_pumps_size = (transfer_pumps_height_diff * 9.81 + (((transfer_pumps_capacity/1000)/transfer_pumps_area) ** 2) / 2 + transfer_pumps_pressure_diff) * transfer_pumps_capacity / (746 * transfer_pumps_motor_eff * transfer_pumps_pump_eff * transfer_pumps_coupling_eff**-1) #convert from watts to hp
    else:
        transfer_pumps_size = 0 #hp
        
    # Product Water Pump (sheet f)
    product_pumps_capacity = plant_production_flow / (product_pumps_number) # L/s per pump
    product_pumps_area = product_pumps_capacity / (1000 * 2.5) # m^2 pipe cross sectional area
    
    if product_pumps == 'Y':
        product_pumps_size = (product_pumps_height_diff * 9.81 + (((product_pumps_capacity/1000)/product_pumps_area) ** 2) / 2 + product_pumps_pressure_diff) * product_pumps_capacity / (746 * product_pumps_motor_eff * product_pumps_pump_eff * product_pumps_coupling_eff**-1) #convert from watts to hp
    else:
        product_pumps_size = 0
        
    # Flow & Water Quality & Membrane & Unit Configuration (sheet f)
    if blending == 'Y':
        blending_bypass_flow = plant_production_flow * (TDS_target - TDS_product) / (TDS_feed - TDS_product) #L/s
    ...
    membrane_flux = pure_water_perm / membrane_area # water flux per unit area of membrane
    water_transport_coeff = membrane_flux / transmembrane_pressure # m/(s*Pa)
    
    # Membrane & Unit Configuration (sheet f) 
    element_productivity = (water_transport_coeff * area_per_module * net_driving_pressure * 1000 * T_coeff) * 1000 # L/s
    number_of_elements = ((membrane_capacity/element_productivity)/(3 * elements_per_vessel))*(3 * elements_per_vessel) # TODO roundup
    ...
    building_cost = building_cost_per_m2 * (building_area + admin_area) * cost_rate_building
    electrical_cost = electrical_cost_per_L * 1000 * membrane_capacity**.65 * cost_rate_construction
    instrument_cost = instrument_base_cost * skids_calculated * cost_rate_construction + 300000  # $300,000 added for top of the line DAC
    ...
    if HPP_style == 'PD':
        ...
        HPP_cost = HPP_cost * 85000 * HPP_size**.65 
    elif HPP_style == 'CSS':
        HPP_cost = HPP_cost * 35000 * HPP_size**.65 
    else:
        HPP_cost = 0
    ...
    if ERG == 'Y':
        if applied_pressure > 1400:
            ERG_cost = 3463.3 * feed_flow_rate**.3906 * cost_rate_construction
        else:
            ERG_cost = 0
    else:
        ERG_cost = 0
    ...
    if transfer_pumps == 'Y':
        if transfer_pumps_style == 'VST':
            transfer_pumps_cost = transfer_pumps_number * cost_rate_piping * (85000 * (transfer_pumps_size / 100) ** .65)
        elif transfer_pumps_style == 'CSS':
            transfer_pumps_cost = transfer_pumps_number * cost_rate_piping * (transfer_pumps_size * (.0387 * (transfer_pumps_capacity * .264 * 60) + 233.86))
        else:
    ...
    if product_pumps == 'Y':
        if product_pumps_style == 'VST':
            product_pumps_cost = product_pumps_number * cost_rate_piping * (85000 * (product_pumps_size / 100) ** .65)
        elif transfer_pumps_style == 'CSS':
            product_pumps_cost = product_pumps_number * cost_rate_piping * (product_pumps_size * (.0387 * (product_pumps_capacity * .264 * 60) + 233.86))
        else:
    ...
    if odor_control == 'Y':
        odor_cost = odor_base_cost * ((feed_flow_rate * recovery_factor * 0.0228) ** .75) * cost_rate_piping
    else:
        odor_cost = 0 
    
    # Process piping cost (sheet g)
    process_piping_cost = process_piping_base_cost * feed_flow_rate * .0228 * cost_rate_piping
    
    # Yard piping cost (sheet g)
    yard_piping_cost = yard_piping_base_cost * (feed_flow_rate * .0228) **.75 * cost_rate_piping

    # Cartridge filters cost (sheet g)
    filters_cost = filters_base_cost * (feed_flow_rate * .0228) **.85 * cost_rate_piping
    ...
    concentrate_treat_pipe_cost = concentrate_cost_per_L * (membrane_capacity * 84600) * (1-recovery_factor) * cost_rate_piping

    # Generator cost (sheet g)
    generator_cost = 150000 * (generator_MW / 1000) ** .85 + 50000 * cost_rate_construction

    #Sitework cost (sheet g)
    sitework_cost = sitework_cost_per_m3 * (membrane_capacity * 84600 / 1000) * cost_rate_labor
    
    total_direct_cap_costs = sum([membrane_cost, RO_skids_cost, building_cost, electrical_cost, instrument_cost, HPP_cost, ERG_cost, transfer_pumps_cost, product_pumps_cost, odor_cost, process_piping_cost, yard_piping_cost, filters_cost, mem_cleaning_cost, contractor_cost, concentrate_treat_pipe_cost, generator_cost, sitework_cost])

    # Indirect Capital Costs
    interest_during_construction = interest_during_construction_percent * total_direct_cap_costs
    ...
    total_indirect_cap_costs = sum([interest_during_construction, contingencies, fees, working_capital])
    ...
    transfer_pumps_energy = transfer_pumps_number * .746 * 8760 * op_time_factor * transfer_pumps_size #kWh
    HPP_pumps_energy = HPP_number * .746 * 8760 * op_time_factor * HPP_size #kWh
    product_pumps_energy = product_pumps_number * .746 * 8760 * op_time_factor * product_pumps_size #kWh
    total_energy_required = sum([transfer_pumps_energy, HPP_pumps_energy, product_pumps_energy])
