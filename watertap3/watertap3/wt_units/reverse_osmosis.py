...
class UnitProcess(WT3UnitProcess):
    ...
    def fixed_cap(self, t, b_cost):
        ...
        '''
        ro_cap = (self.tpec_tic * (b_cost.pump_capital_cost + b_cost.mem_capital_cost + b_cost.erd_capital_cost) + 3.3 * (self.pressure_vessel_cost1[t] + self.rack_support_cost1[t])) * 1E-6  # $MM ### 1.65 is TIC
        return ro_cap
    ...
    def _set_flow_mass(self, b):
        ...
        b.mass_flow_H2O = Var(time, domain=NonNegativeReals, units=self.units_meta('mass') / self.units_meta('time'), doc='mass flow rate')
        b.mass_flow_tds = Var(time, domain=NonNegativeReals, units=self.units_meta('mass') / self.units_meta('time'), doc='mass flow rate')

    def _set_mass_frac(self, b):
        time = self.flowsheet().config.time
        b.mass_frac_H2O = Var(time, units=pyunits.dimensionless, doc='mass_fraction')
        b.mass_frac_tds = Var(time, units=pyunits.dimensionless, doc='mass_fraction')

    def _set_pressure_osm(self, b):
        time = self.flowsheet().config.time
        b.pressure_osm = Var(time, domain=NonNegativeReals, doc='Osmotic pressure')

    def _set_osm_coeff(self, b):
        time = self.flowsheet().config.time
        b.osm_coeff = Var(time, initialize=0.1, units=pyunits.dimensionless, doc='Osmotic pressure coefficient')

    def _set_conc_mass(self, b):
        time = self.flowsheet().config.time
        b.conc_mass_H2O = Var(time, domain=NonNegativeReals, units=self.units_meta('mass') / self.units_meta('volume'), doc='H2O mass density')
        b.conc_mass_total = Var(time, domain=NonNegativeReals, units=self.units_meta('mass') / self.units_meta('volume'), doc='density')

    def _set_pressure(self, b):
        time = self.flowsheet().config.time
        b.pressure = Var(time, initialize=45, domain=NonNegativeReals, bounds=(2, 90), doc='pressure')
    ...
    def _set_constraints(self, t):
        ...
        self.flow_vol_eq1 = Constraint(expr=self.flow_vol_out[t] * self.permeate.conc_mass_total[t] == (self.permeate.mass_flow_tds[t] + self.permeate.mass_flow_H2O[t]))
        self.flow_vol_eq2 = Constraint(expr=self.flow_vol_waste[t] * self.retentate.conc_mass_total[t] == (self.retentate.mass_flow_tds[t] + self.retentate.mass_flow_H2O[t]))
        self.pressure_waste_outlet_eq = Constraint(expr=self.feed.pressure[t] - self.pressure_drop == self.pressure_waste[t])

        # MASS BALANCE
        self.mass_balance_H2O = Constraint(expr=self.feed.mass_flow_H2O[t] == self.permeate.mass_flow_H2O[t] + self.retentate.mass_flow_H2O[t])
        self.mass_balance_tds = Constraint(expr=self.feed.mass_flow_tds[t] == self.permeate.mass_flow_tds[t] + self.retentate.mass_flow_tds[t])

        # PERMEATE PRESSURE
        self.p_out_eq = Constraint(expr=1 == self.pressure_out[t])
        self.pump_constraint_power = Constraint(expr=self.pump_power >= 0)

        # VESSEL COST
        self.pressure_vessel_cost1_eq = Constraint(expr=self.pressure_vessel_cost1[t] * 0.99 <= self.membrane_area[t] * 0.025 * 1000) # assumes 2 trains. 150 ft start, 5ft per additional vessel. EPA.
        self.rack_support_cost1_eq = Constraint(expr=self.rack_support_cost1[t] * 0.99 <= (150 + (self.membrane_area[t] * 0.025 * 5)) * 33 * 2)
        self.pressure_vessel_cost1_eq2 = Constraint(expr=self.pressure_vessel_cost1[t] * 1.01 >= self.membrane_area[t] * 0.025 * 1000) # assumes 2 trains. 150 ft start, 5ft per additional vessel. EPA.
        self.rack_support_cost1_eq2 = Constraint(expr=self.rack_support_cost1[t] * 1.01 >= (150 + (self.membrane_area[t] * 0.025 * 5)) * 33 * 2)
    ...
    def get_costing(self, unit_params=None, year=None):
        ...
        for j in self.const_list2:
            if scaling == 'yes':
                ...
                setattr(self, ("%s_eq1" % j), Constraint(expr=self.flow_vol_in[t] * self.conc_mass_in[t, j] == self.flow_vol_out[t] * self.conc_mass_out[t, j] + self.flow_vol_waste[t] * self.conc_mass_waste[t, j]))
                # if j in ['toc', 'sulfur', 'nitrate_as_nitrogen']:
                setattr(self, ("%s_eq" % j), Constraint(expr=self.removal_fraction[t, j] * self.flow_vol_in[t] * self.conc_mass_in[t, j] == self.flow_vol_waste[t] * self.conc_mass_waste[t, j]))
